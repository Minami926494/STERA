...
XH = (('样式整理', r'(?<!ctt )class="(?:(?!ctt).)*?"',
      ...
      ('章节重构', '',
       ('目录整理', {
           r'<ctt[^>\n]*?>[\s\S]*<a[^>\n]*?>\s*<p>(?:目录|目錄|目録|目次|CONTENTS|Contents|もくじ|もくろく)(?:页|頁|ページ)?</p>\s*</a>\s*</ctt>\s*': '',
           r'<ctt[^>\n]*?>[\s\S]*<a[^>\n]*?>\s*<p>(?:書封|书封|封面|書名|书名|表纸|表紙|ひょうし)(?:页|頁|ページ)?</p>\s*</a>\s*</ctt>\s*': '',
           r'\s*<ctt[^>\n]*?>\s*<a[^>\n]*?>\s*<p>(?:版權|版权|奥付|おくづけ)(?:页|頁|ページ)?</p>\s*</a>[\s\S]*</ctt>': '',
           r'\s*<ctt[^>\n]*?>\s*<a[^>\n]*?>\s*<p>(?:封底|里表纸|裏表紙|うらびょうし)(?:页|頁|ページ)?</p>\s*</a>[\s\S]*</ctt>': '',
           r'(<ctt[^>\n]*?>\s*<a[^>\n]*?>)\s*<p>([\s　]+)(.*?)(?(2)[\s　]*|[\s　]+)</p>\s*</a>': r'\1\3</a>',
           r'(<ctt[^>\n]*?>\s*<a[^>\n]*?>\s*<p>\S)[\s　]+(\S(?:[\s　]+.*?)?</p>\s*</a>)': r'\1\2'}),
       ('标题拉取', {
           ...
           r'(*)<ctt([^>\n]*?>\s*<a[^>\n]*?href="(?:[^"\n]*/)?(.*?\.xhtml)[^>\n]*?>\s*<p>([^<>\n]*?)</p>\s*</a>[\s\S]*?<page.*?href="\2"(?:(?!<body>)[\s\S])*?<title>).*?(</title>(?:(?!<body>)[\s\S])*?<body>)': r'<div\1\3\4\n<h7>\3</h7>',
           r'(</?)ctt([^>\n]*?>)': r'\1div\2',
           ...
           r'(<a[^>\n]*?>\s*<p>.*?)　+(.*?</p>\s*</a>)': r'\1</p><p class="ctit">\2'}),
       ('标题去重', {
           r'(?<=</h7>)(?:(?!</body>)[^一-龥あ-ヶー])*\n(.*?[一-龥あ-ヶー].*)$$': r'\n<chapter>\1</chapter>\1',
