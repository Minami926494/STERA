...
XH = (('样式整理', r'(?<!ctt )class="(?:(?!ctt).)*?"',
      ...
      ('章节重构', '',
       ('目录整理', {
           r'<ctt.*?>[\s\S]*<a[^>\n]*?>\s*(?:目录|目錄|目録|目次|CONTENTS|Contents|もくじ|もくろく)(?:页|頁|ページ)?\s*</a>\s*</ctt>\s*': '',
           r'<ctt.*?>[\s\S]*<a[^>\n]*?>(?:書封|书封|封面|書名|书名|表纸|表紙|ひょうし)(?:页|頁|ページ)?</a>\s*</ctt>\s*': '',
           r'\s*<ctt.*?>\s*<a[^>\n]*?>(?:版權|版权|奥付|おくづけ)(?:页|頁|ページ)?</a>[\s\S]*</ctt>': '',
           r'\s*<ctt.*?>\s*<a[^>\n]*?>(?:封底|里表纸|裏表紙|うらびょうし)(?:页|頁|ページ)?</a>[\s\S]*</ctt>': '',
           r'(<ctt.*?>\s*<a[^>\n]*?>)([\s　]+)(.*?)(?(2)[\s　]*|[\s　]+)</a>': r'\1\3</a>',
           r'(<ctt.*?>\s*<a[^>\n]*?>\S)[\s　]+(\S(?:[\s　]+.*?)?</a>)': r'\1\2'}),
       ('标题拉取', {
           ...
           r'(*)<ctt(.*?>\s*<a[^>\n]*?href="(?:[^"\n]*/)?(.*?\.xhtml)[^>\n]*?>([^<>\n]*?)</a>)([\s\S]*?)(<page.*?href="\2[\s\S]*?<title>).*?(</title>[\s\S]*?<body>)': r'<div\1\4\5\3\6\n<h7>\3</h7>',
           r'<ctt.*?>\s*<a.*?/a>\s*(?:<[^>\n]*?class="part">\s*<a.*?/a>\s*</ctt>\s*)*</ctt>\s*': '',
           '</ctt>': '</div>',
           ...
           r'(<div class="(?:toc|part)">\s*<a[^>\n]*?>)(.*?)　+(.*?)(?=</a>)': r'\1<p>\2</p><p class="ctit">\3</p>',
           r'(<div class="(?:toc|part)">\s*<a[^>\n]*?>)((?:(?!<p).)*?)(?=</a>)': r'\1<p>\2</p>',
           r'</div>\s*<div class="part">': ''}),
       ('标题去重', {
           r'(?<=</h7>)(?:(?!</body>)[^一-龥あ-ヶー])*\n(.*?[一-龥あ-ヶー].*)$$': r'\n<chapter>\1</chapter>\1',
           r'(<h7>(\S+?)[ 　]*(.*?)</h7>\n<chapter>.*?</chapter>.*$$(?:(?!</body>)[\s\S])*?)^^(.*?\2.*?\3.*)$$': r'\1<chapter>\4</chapter>\4',
       ...
       ('标题样式', {
           ...
           r'<h7>(.+?)((～|-|—|〈)+.+(?:\3|〉)+)</h7>': r'<h4>\1</h4>\n<h5>\2</h5>',
           r'<(h\d)>(.*?)\s*(（.*）)</\1>': r'<\1>\2<span class="em08">\3</span></\1>',
           r'<h7>(.*)</h7>': r'<h3>\1</h3>'})),
      ('注释生成', '',
       ('行内注释', {
